...
- \`"soi"\`:
  ...
  backfilled tax-year rows from the 2020+ calendar-year extracts (no
  original calendar-year 2017-2019 PF SOI was published); row counts are
  small (~665, 3.3k, 100k). For provenance-aware reads, use the
  \`"merged"\` tier (which carries \`source_pipeline\` and
  \`has_legacy_augment\`).
