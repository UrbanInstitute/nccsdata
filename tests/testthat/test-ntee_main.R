...
test_that("ntee_preview() filters Level 1 correctly", {
  ...
  for (i in level1){
  expect_match(quiet(ntee_preview(ntee = i)), i)
  expect_match(quiet(ntee_preview(ntee = i)), i)
  }
})
...
test_that("ntee_preview() filters Level 2 correctly", {
  ...
  for (lvl in level2){
    ...
    expect_match(quiet(ntee_preview(ntee = lvl)), rgxp)
    expect_match(quiet(ntee_preview(ntee.code = lvl)), rgxp)
  }
})
...
test_that("ntee_preview() filters Levels 2-4 correctly", {
  ...
  for (lvl in level24){
    expect_match(quiet(ntee_preview(ntee = lvl)), lvl)
    expect_match(quiet(ntee_preview(ntee.code = lvl)), lvl)
  }
})
...
test_that("ntee_preview() filters Level 5 correctly", {
  ...
  for (i in level5){
    expect_match(quiet(ntee_preview(ntee = i)), i)
    expect_match(quiet(ntee_preview(ntee.orgtype = i)), i)
  }
})
