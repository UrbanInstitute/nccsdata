...
Multi-year panels mix nominal dollars from different years. Before
comparing or aggregating across years, deflate to a common base year
with
[`nccs_deflate()`](https://urbaninstitute.github.io/nccsdata/reference/nccs_deflate.md),
which uses the bundled annual CPI-U series (`cpi_u`):

``` r

panel_real <- panel |>
  mutate(
    total_revenue_2023  = nccs_deflate(as.numeric(total_revenue),
                                       year = as.integer(substr(tax_period, 1, 4)),
                                       base_year = 2023),
    total_expenses_2023 = nccs_deflate(as.numeric(total_expenses),
                                       year = as.integer(substr(tax_period, 1, 4)),
                                       base_year = 2023)
  )
```

The package deliberately stops at deflation — canonical financial ratios
(operating margin, program-expense ratio, fundraising efficiency, etc.)
are one-line
[`mutate()`](https://dplyr.tidyverse.org/reference/mutate.html) calls
whose definitions vary by analyst, so we don’t ship opinionated
wrappers.
