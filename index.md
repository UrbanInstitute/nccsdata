...
downloads BMF data from S3 with optional filters. Filtering happens at
the Arrow level via predicate pushdown, so only matching rows are read
into memory.
...
[`nccs_read()`](https://urbaninstitute.github.io/nccsdata/reference/nccs_read.md)
coerces known financial, date, and indicator columns to their natural
types on the collected tibble by default — pass `coerce = FALSE` to opt
out. ZIPs and other ID-like codes are intentionally left as `character`.

### Local caching

[`nccs_read()`](https://urbaninstitute.github.io/nccsdata/reference/nccs_read.md)
caches the geocoded master parquet (hundreds of MB) in
`tools::R_user_dir("nccsdata", "cache")` so subsequent calls in the same
or future sessions skip the S3 download. The cached copy refreshes once
it’s more than `cache_max_age` days old (30 by default; the upstream
master is rebuilt monthly). Pass `cache = FALSE` to always read from S3,
or use
[`nccs_cache_clear()`](https://urbaninstitute.github.io/nccsdata/reference/nccs_cache_dir.md)
to force the next call to re-download.
...
[`nccs_read()`](https://urbaninstitute.github.io/nccsdata/reference/nccs_read.md)
reads the rolling geocoded master. For a specific dated monthly snapshot
— useful for reproducible research — use
[`nccs_vintage_url()`](https://urbaninstitute.github.io/nccsdata/reference/nccs_vintage_url.md)
to get the S3 URI, then read the CSV directly. Vintage schemas differ by
month and across the modern/legacy seam, so inspect the matching data
...
produces grouped count summaries from a collected data frame.
...
[`nccs_read()`](https://urbaninstitute.github.io/nccsdata/reference/nccs_read.md)
filters without any network calls.
...
The BMF returned by
[`nccs_read()`](https://urbaninstitute.github.io/nccsdata/reference/nccs_read.md)
is already normalized upstream, but two helpers are exposed for users
joining external CSVs or API responses against it:
...
|----|----|
| `get_data()` | [`nccs_read()`](https://urbaninstitute.github.io/nccsdata/reference/nccs_read.md) |
| `preview_sample()` | [`nccs_summary()`](https://urbaninstitute.github.io/nccsdata/reference/nccs_summary.md) |
