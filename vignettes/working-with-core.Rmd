---
title: "Working with the CORE Series (Form 990 filings)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Working with the CORE Series (Form 990 filings)}
...
- [CORE data catalog](https://nccs.urban.org/nccs/catalogs/catalog-core.html)

This package exposes four CORE helpers — `nccs_core_url()`,
`nccs_read_core()`, `nccs_core_columns()`, `nccs_core_coverage()` — and
this vignette shows the workflows they're designed for.
...
`has_legacy_augment` — that let you trace any row back to its
upstream source if you need to.
...
`nccs_core_columns()` to see what's available without downloading the
filings parquet itself.
...
`nccs_core_url()` returns the canonical S3 URL for any partition or
dictionary file if you want to inspect it outside R:
...
`nccs_read_core()` reads a single `(tier, tax_year, form)` partition
with column projection (only requested columns leave S3) and local
caching by default.
...
still push down across all of them. Before downloading anything it
isn't already caching, the function reports the total transfer size and
(in interactive sessions) prompts:
...
local cache instead of S3. The cache lives under `nccs_cache_dir()` —
see `?nccs_cache_clear` if you need to wipe it.

`nccs_core_coverage()` lists every published partition for a tier with
row counts, which is useful both as a sanity check and for spotting the
small / backfilled partitions:
...
with `nccs_deflate()`, which uses the bundled annual CPI-U series
(`cpi_u`):
...
The package deliberately stops at deflation — canonical financial
ratios (operating margin, program-expense ratio, fundraising
efficiency, etc.) are one-line `mutate()` calls whose definitions vary
by analyst, so we don't ship opinionated wrappers.
...
`nccs_normalize_ein()` first so they match the canonical form used by
both CORE and BMF:
...
- Most columns in CORE parquet land as `character` to keep schemas
  stable across vintages — cast numeric and date columns at the
  consumer (`as.numeric()`, `as.Date()`). Use `nccs_as_indicator()` for
  the IRS Yes/No fields.
- `nccs_core_coverage()` issues one HTTP request per partition (footer
  metadata only, no full scan) and typically takes 10-60 seconds per
  tier.
- The CORE catalog at
