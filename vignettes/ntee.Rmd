...
explore and query NTEE codes with the help `preview_ntee()` and `parse_ntee()`.
...
`preview_ntee()` then prints out the descriptions of all NTEE codes belonging
...
After understanding what the NTEE codes represent, with `preview_ntee()`, 
...
Similar to `preview_ntee()`, `parse_ntee()` accepts placeholder arguments.
